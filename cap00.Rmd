...
## Git, GitHub, R, RStudio y RMardown
...
A continuación explicamos qué se debe instalar, siendo primero ciertas aplicaciones:

#### _Software_

Debemos descargar __Git__ desde [Descargas](https://git-scm.com/downloads). Lo instalamos. Enseguida descargamos __R__ desde el repositorio [CRAN](https://cran.itam.mx/) que corresponde a nuestro país (México). En esta página se encuentran versiones para los tres sistemas operativos más utilizados. En el caso de _Windows_, existen pasos adicionales para su descarga:

1. Seleccionamos el enlace _Download R for Windows_:
![](images/001-r.png)

2. Enseguida damos clic en el enlace _Install R for the first time_:
![](images/002-r.png)

3. Y terminamos con la descarga del instalador:
![](images/003-r.png)

La instalación es normal, incluso sin modificar ninguna opción por defecto.

Ahora bajamos e instalamos __RStudio__ desde [Descargas](https://www.rstudio.com/products/rstudio/download/), cuidando que sea obviamente la versión deseada:

![](images/001-rstudio.png)

Para el sistema operativo adecuado:

![](images/002-rstudio.png)

Lo ejecutamos enseguida. Queda instalar algunas librerías necesarias para la creación de nuestros documentos. 

#### Librerías necesarias

Aunque los programas instalados ya pueden ser utilizados, falta instalar algunos paquetes que añaden funcionalidad y lo necesario para poder elaborar nuestros documento y páginas web. Procedamos a agregarlas a _RStudio_.

##### _RMarkdown_

En primera instancia, debemos instalar `RMarkdown`. Esto se realiza yendo al menú _Tools_ y dar clic sobre la opción _Install packages_:

![](images/001-rmarkdown.png)

Aparecerá una ventana, en la cual escribiremos el nombre del paquete a instalar: `rmarkdown`. Notemos que mientras vamos escribiendo el nombre, RStudio nos sugiere posibles paquetes, podemos seleccionar el deseado desde que lo muestra.

![](images/002-rmarkdown.png)

Ya seleccionado damos clic en _Install_, asegurándonos de que la casilla _Install dependencies_ esté seleccionada:

![](images/003-rmarkdown.jpg)

Esperamos a que descargue los archivos necesarios y los instale. Veremos algunas barras de descarga. Sabremos que ya ha terminado cuando aparezca el siguiente mensaje en la ventana _Console_:

![](images/004-rmarkdown.png)

##### Tinytex

Quizá necesitemos generar algunos documentos _PDF_ en el camino. Para ello, hay que instalar alguna distribución _Latex_. Sin embargo, pueden resultar bastante pesadas, llegando incluso a requerir descargas de hasta 1GB. Debido a este inconveniente, existe una distribución ligera que contiene solo lo necesario para la generación de documentos técnicos: _Tinytex_. La instalaremos en _RStudio_ de la siguiente manera:

En el panel de la consola (Console) comenzaremos a escribir `tinytex`. Si mientras lo escribimos aparece un recuadro flotante con la palabra _tinitex::_, tal como se muestra en la siguiente imagen, significa que está instalado.
![](images/001-tinytex.png)

En caso contrario, toca instalarlo mediante un proceso exactamente igual al que usamos cuando instalamos _RMarkdown_. Terminando, volvemos a probar si aparece el recuadro. Al hacerlo, damos `Enter` y aparecerá un menú contextual con varias opciones. Seleccionamos _install_tinytex_:

![](images/002-tinytex.png)

Al haberlo hecho, debe quedar lo siguiente en consola:

![](images/003-tinytex.png)

Damos `Enter` y el programa comenzará a instalar la distribución. Tardará un rato, ya que deberá descargar, compilar e instalar. Sin embargo, lanzará un aviso como el que se muestra:

![](images/004-tinytex.png)

Básicamente, avisa que podrían aparecer dos ventanas de error y que no pasa nada, que felizmente las ignoremos dando clic en `Aceptar`. Después de un rato, hayan o no salido los mensajes de error, el programa habrá terminado con la siguiente frase, pidiendo que cerremos _RStudio_ y volvamos a abrirlo:

![](images/005-tinytex.png)

Nuestro entorno de trabajo está listo para generar documentos estadísticos por medio de RStudio.
...
Es momento de instalar los paquetes que son exactamente para el _análisis de datos_, el _machine learning_ y el _big data_. Para ello utilizaremos el mismo proceso para instalar que el visto anteriormente. En la ventana que aparece tras dar clic en _Tools -> Install packages_ escribiremos `tidyverse`:

![](images/001-tidyverse.png)
 
Esperamos a que descargue:
 
![](images/002-tidyverse.png)
 
e instale. Lo sabremos porque tendremos una pantalla muy similar a la siguiente:
 
![](images/003-tidyverse.png)
 
Sin embargo, es necesario cargar los paquetes que hemos instalado. Para ello se utiliza el comando `library(Nombre_del_paquete)`. Aunque los paquetes deban instalarse una sola vez, siempre deben ser cargados al iniciar sesión. Escribimos pues:

![](images/004-tidyverse.png)

Y al ejecutarlo dando `Enter` obtendremos la siguiente información en pantalla, mostrando los paquetes instalados y cargados:

![](images/005-tidyverse.png)
 
## Conjuntos de datos de ejemplo o _datasets_

Muchos de los conjuntos de datos analizados en estas páginas pertenecen al siguiente paquete instalable desde internet: `datos`. Es muy recomendable instalarlo, junto con otras dependencias. Primero instalemos `datos`:

![](images/006-tidyverse.png)

Enseguida instalamos las dependencias del paquete. Pero como serán varios que debemos instalar, escojamos otro método para hacerlo, más rápido en cuanto a su captura. En el panel de la consola escribiremos lo siguiente:

```
install.packages(c("nycflights13", "gapminder", "Lahman", "nasaweather", "babynames", "fueleconomy"))
```
Esperemos que finalice la descarga y ya tenemos listas nuestras herramientas de trabajo.
